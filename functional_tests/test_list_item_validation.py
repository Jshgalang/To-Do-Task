    ...
    def test_cannot_add_duplicate_items(self):
        ...
        self.wait_for(
            lambda: self.assertEqual(
                self.get_error_element(), "You've already got this on your list"))

    def test_error_messages_are_cleared_on_input(self):
        # Mike starts a lsit and ignites a validation error
        self.browser.get(self.live_server_url)
        self.get_item_input_box().send_keys('Payongayong')
        self.get_item_input_box().send_keys(Keys.ENTER)
        self.wait_for_row_in_list_table('1: Payongayong')
        self.get_item_input_box().send_keys('Payongayong')
        self.get_item_input_box().send_keys(Keys.ENTER)
        self.wait_for(
        ...
        # Mike starts typing in the input box to clear the error
        self.get_item_input_box().send_keys('a')
        # Mike sees error message disappear
        self.wait_for(
            lambda: self.assertFalse(
            self.get_error_element().is_displayed()
            )) #  checks element if visible
